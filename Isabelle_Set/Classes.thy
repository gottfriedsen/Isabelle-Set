theory Classes
  imports Lifting_Set_a Monoid_Set HOL.BNF_Def HOL.Sledgehammer
begin
...
definition "monoid_rel R M N \<equiv>
  (\<exists>A. M : Monoid A \<and> sup_dom R A) \<and> (\<exists>B. N : Monoid B \<and> sup_rng R B) \<and>
...
proof
  ...
  obtain S where A_type [type]: "A : Monoid S" and S_sup_R_dom: "sup_dom R S"
    using monoid_rel_A_B unfolding monoid_rel_def by blast
  obtain T where B_type: "B : Monoid T" and T_sup_R_rng: "sup_rng R T"
    using monoid_rel_A_B unfolding monoid_rel_def by blast
