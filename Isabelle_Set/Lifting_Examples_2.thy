...
lemma int_div_type': "int_div: (i : Int') \<Rightarrow> (divides i \<sqdot> Int') \<Rightarrow> Int'"
...
lemma
  assumes "i : Int'" "j : Int'"
  shows "ite (divides i j) (int_div i j) (i) : Int'"
  apply (rule Fun_typeE[where f="ite (divides i j) (int_div i j)" and x=i and B=Int'])
  prefer 2
  apply (rule Fun_typeE[where f="ite (divides i j)" and x="int_div i j"])
  prefer 2
  apply (rule dep_fun_typeE[where f=ite and x="divides i j"])
  prefer 2
  apply (rule ite_type[where \<alpha>=Int'])
...
definition "two_rep \<equiv> int_rep_add (inl 1) (inl 1)"
...
lemma even_type: "even : Int' \<Rightarrow> Element Bool_set"
proof -
  have u1elem: "\<And>i. i : Element int_rep \<Longrightarrow> even_rep i : Element Bool_set"
    sorry
  show "even : Int' \<Rightarrow> Element Bool_set"
...
lemma even_cases_type': "even_cases : (Int' \<Rightarrow> Int') \<Rightarrow> (Int' \<Rightarrow> Int') \<Rightarrow> Int' \<Rightarrow> Int'"
proof -
  have u1elem: "\<And>f g x. (\<And>x. x : Element int_rep \<Longrightarrow> f x : Element int_rep) \<Longrightarrow> (\<And>x. x : Element int_rep \<Longrightarrow> g x : Element int_rep) \<Longrightarrow> x : Element int_rep \<Longrightarrow> even_cases_rep f g x : Element int_rep"
    (* left to the user *)
    sorry
  show "even_cases : (Int' \<Rightarrow> Int') \<Rightarrow> (Int' \<Rightarrow> Int') \<Rightarrow> Int' \<Rightarrow> Int'"
    apply (rule Fun_typeI)+
    apply (subst even_cases_def)
    apply (subst Element_abs)
...
lemma "int_app2 : ((Int' \<Rightarrow> Int') \<Rightarrow> Int' \<Rightarrow> Int') \<Rightarrow> (Int' \<Rightarrow> Int') \<Rightarrow> Int' \<Rightarrow> Int'"
proof (
      rule type_from_rel_fun2'[OF
        ...
        Int_Rel_app2])
  fix f
  assume f_type: "f : (Int' \<Rightarrow> Int') \<Rightarrow> Int' \<Rightarrow> Int'"
  note 1 = Fun_typeE[OF _ f_type]
  { fix x y
    assume eq_x_y: "(eq \<int> ===> eq \<int>) x y"
    have x_type: "x : Int' \<Rightarrow> Int'"
    ...
    have y_type: "y : Int' \<Rightarrow> Int'"
      apply (rule type_from_in_abs_fun[OF Int_Rel_lifting Int_Rel_lifting])
...
next
  ...
  show "int_app2 x : (Int' \<Rightarrow> Int') \<Rightarrow> Int' \<Rightarrow> Int'"
    sorry
qed
