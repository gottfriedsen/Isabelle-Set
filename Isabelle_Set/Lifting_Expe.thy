...
lemma Rep_eq:
  assumes set_ext: "set_extension A B f" "x \<in> set_extension.def A B f" "y \<in> set_extension.def A B f"
  shows "(set_extension.Rep A f x = set_extension.Rep A f y) = (x = y)"
proof (rule HOL.iffI)
  assume "set_extension.Rep A f x = set_extension.Rep A f y"
  thus "x = y" using assms set_extension.Rep_inverse by metis
next
  assume "x = y"
...
lemma Rel:
  ...
  defines "Rep \<equiv> set_extension.Rep A f"
    ...
    and "def \<equiv> set_extension.def A B f"
...
next
  show "Abs x = y"
    using set_extension.Rep_inverse axioms rel by (metis Rel_def Rep_def def_def Abs_def)
next
  show "y \<in> def"
    using rel by (meson Rel_def)
next
...
next
  show "y \<in> B \<Longrightarrow> Rel def Rep y (Abs y)"
    unfolding Rel_def
    using set_extension.Abs_type[OF axioms] set_extension.Abs_inverse[OF axioms]
...
lemma rat_pow_type'':
  assumes types: "n : Nat" "i : Int'"
  shows "int_pow n i : Int'"
  apply (rule type_from_rel_elem)
  apply (rule Int_Rel_def')
...
lemma int_div_type'':
  assumes types: "i : Int'" "j : Int'"
    and refinements: "divides i j"
  shows "int_div i j : Int'"
  apply (rule type_from_rel_elem)
  apply (rule Int_Rel_def')
  apply (rule Int_Rel_div')
  apply (subst Int_Rel_def')
