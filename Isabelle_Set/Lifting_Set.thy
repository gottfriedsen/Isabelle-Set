theory Lifting_Set
  imports HOL.BNF_Def Set_Extension Dep_Rel Atomize HOL.Sledgehammer
begin
...
lemma bi_uniqueE: "\<lbrakk>bi_unique_set R; R x x'; R y y'\<rbrakk> \<Longrightarrow> x = y \<longleftrightarrow> x' = y'"
  unfolding bi_unique_set_def left_unique_set_def right_unique_set_def sball_def
  by blast
...
lemma set_extension_lifting:
  ...
  defines "Rep \<equiv> set_extension.Rep A f"
    ...
    and "abs \<equiv> set_extension.def A rep f"
  ...
  using axioms Abs_def Fun_typeE'
    set_extension.Rep_type set_extension.Abs_type set_extension.Rep_inverse set_extension.Abs_inverse
  apply metis+
  done
...
proof ((subst lifting_unfold rel_fun_def)+, (rule conjI)?; (rule conjI)?; (rule conjI)?; (rule conjI)?; (rule conjI)?; (rule conjI)?; (rule conjI)?; (rule conjI)?; (rule sballI | rule allI)+)
  show
...
lemma type_from_rel_ext1: "set_extension A B f \<Longrightarrow> R = Ext_Rel (set_extension.def A B f) (set_extension.Rep A f) \<Longrightarrow> R x y \<Longrightarrow> x : Element B"
  apply unfold_types
  unfolding Ext_Rel_def
...
definition "Bool_set \<equiv> Sum.sum {\<emptyset>} {\<emptyset>}"
...
definition "bool_set_case t f \<equiv> sum_case (\<lambda>_. t) (\<lambda>_. f)"
...
lemma bool_set_case_type: "bool_set_case : A \<Rightarrow> A \<Rightarrow> Element Bool_set \<Rightarrow> A"
  ...
  apply (rule Fun_typeE[OF _ Fun_typeE[OF _ Fun_typeE[OF _ sum_case_type]]])
