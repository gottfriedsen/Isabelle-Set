...
definition "list_rep_case n c \<equiv> sum_case (\<lambda>_. n) (\<lambda>p. c (fst p) (snd p))"
...
axiomatization list_rep_rec :: "(set \<Rightarrow> set) \<Rightarrow> (set \<Rightarrow> set \<Rightarrow> set \<Rightarrow> set \<Rightarrow> set) \<Rightarrow> (set \<Rightarrow> set \<Rightarrow> set \<Rightarrow> set) \<Rightarrow> set \<Rightarrow> set \<Rightarrow> set"
  where list_rep_rec_nil: "list_rep_rec n c f a Nil_rep = n a"
    ...
    list_rep_rec n c f a (Cons_rep x xs) = sum_case (\<lambda>b. c x xs a b) (\<lambda>b. c x xs a (list_rep_rec n c f b xs)) (f x xs a)"

lemma list_rep_rec_type [type]:
  "list_rep_rec : (Element X \<Rightarrow> Element Y) \<Rightarrow> (Element A \<Rightarrow> Element (List_rep A) \<Rightarrow> Element X \<Rightarrow> Element Y \<Rightarrow> Element Y) \<Rightarrow> (Element A \<Rightarrow> Element (List_rep A) \<Rightarrow> Element X \<Rightarrow> Element (Sum.sum Y X)) \<Rightarrow> Element X \<Rightarrow> Element (List_rep A) \<Rightarrow> Element Y"
proof (intro Pi_typeI)
  fix n c f a xs
  assume assms[type]:
    ...
    "f : Element A \<Rightarrow> Element (List_rep A) \<Rightarrow> Element X \<Rightarrow> Element (Sum.sum Y X)"
    "a : Element X"
...
definition "List_Rel A R \<equiv> Param_Ext_Rel List.def List.Rep list_all2' A R"
