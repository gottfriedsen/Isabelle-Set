...
abbreviation "Sum A B \<equiv> Element (sum A B)"

lemma option_in_sum_type [type]: "option_in_sum : Option A \<Rightarrow> Sum {unit} A"
  apply unfold_types
  unfolding option_in_sum_def
  using option_case_type' by simp

interpretation Sum_Unit: set_extension "option A" "sum {unit} A"  option_in_sum
proof
...
abbreviation "sum_unit \<equiv> Sum_Unit.def"
abbreviation "Sum_Unit A \<equiv> Element (sum_unit A)"

lemmas option_subset_sum_unit [simp] = Sum_Unit.extension_subset
