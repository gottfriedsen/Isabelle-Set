...
proof
  show "int_in_poly : Int' \<Rightarrow> Element poly_rep" by auto
  show "\<forall>x \<in> \<int>. \<forall>y \<in> \<int>. int_in_poly x = int_in_poly y \<longrightarrow> x = y"
    unfolding int_in_poly_def by auto
qed

abbreviation poly where "poly \<equiv> Poly.def"
abbreviation "Poly \<equiv> Element poly"

lemmas int_subset_poly [simp] = Poly.extension_subset

corollary [derive]: "n : Int' \<Longrightarrow> n : Poly"
...
definition "poly_rep_add p q = \<Union>{
    ...
    let c = a + b
    in if c = 0 then {} else {\<langle>c, n\<rangle>}.
  n \<in> \<nat>}"
...
definition "Poly_Rel p_rep p \<equiv> p \<in> Poly.def \<and> Poly.Rep p = p_rep"

lemma Poly_Rel_0 [transfer_rule]: "Poly_Rel {} 0"
  unfolding Poly.Rep_def Poly_Rel_def Poly.def_def
  using Int_Rel_0 Int_Rel_def int_in_poly_def poly_rep_def
  by simp

lemma Poly_Rel_eq [transfer_rule]: "(Poly_Rel ===> Poly_Rel ===> (=)) (=) (=)"
  unfolding rel_fun_def Poly_Rel_def Poly.Rep_def Poly.def_def
  by (metis Poly.Rep_def Poly.Rep_inverse Poly.def_def)

lemma Poly_Rel_add [transfer_rule]: "(Poly_Rel ===> Poly_Rel ===> Poly_Rel) poly_rep_add poly_add"
  unfolding  rel_fun_def Poly_Rel_def
  using poly_add_def Poly.Abs_inverse poly_add_type
  by (metis (no_types, lifting) ElementD ElementI Pi_typeE)

lemma Poly_Rel_All [transfer_rule]:
  "((Poly_Rel ===> (=)) ===> (=)) (ball poly_rep) (ball poly)"
  unfolding rel_fun_def Poly_Rel_def
  by (smt (verit, ccfv_threshold) ElementD ElementI Pi_typeE Poly.Abs_inverse Poly.Abs_type Poly.Rep_type ball_def)
